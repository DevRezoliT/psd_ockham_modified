 ...
 * https://github.com/DevRezoliT?tab=repositories
 * 
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_DROPFILES:
	{
		...
		{
			wchar_t wbuf[MAX_FILESTRING];
			if (DragQueryFileW(hDrop, i, wbuf, MAX_FILESTRING) != 0)
			{
...
static std::vector<std::string> ListDirectory(const std::string& dir)
{
	...
	WIN32_FIND_DATAW fd;
	HANDLE hFind = ::FindFirstFileW(Utf8ToWide(search_path).c_str(), &fd);
	if (hFind != INVALID_HANDLE_VALUE)
	{
		std::string fileName;
		do
		{
			fileName = WideToUtf8(fd.cFileName);
...
static void FilterFiles(const std::vector<std::string>& files, std::vector<std::string>& result)
{
	for (auto it: files)
	{
		DWORD attr = GetFileAttributesW(Utf8ToWide(it).c_str());
